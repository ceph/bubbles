...
describe('GlassValidators', () => {
  ...
  describe('hostAddress', () => {
    ...
    beforeEach(() => {
      ...
      control?.setValidators(CbValidators.hostAddress());
    });
  ...
  describe('unique', () => {
    ...
    beforeEach(() => {
      control = formGroup.get('x');
      control?.setAsyncValidators(CbValidators.unique((value) => of(value === 'test_value')));
    });
  ...
  describe('requiredIf', () => {
    ...
    const validator = CbValidators.requiredIf({
      operator: 'eq',
      arg0: { prop: 'enabled' },
      arg1: true
    });
    const validator2 = CbValidators.requiredIf({
      operator: 'truthy',
      arg0: { prop: 'enabled' }
    });
  ...
  describe('constraint', () => {
    let enabledControl: AbstractControl;
    const validator = CbValidators.constraint(
      {
        operator: 'truthy',
        arg0: { prop: 'enabled' }
      },
